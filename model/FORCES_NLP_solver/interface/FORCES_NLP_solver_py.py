...
#_lib = ctypes.CDLL(os.path.join(os.getcwd(),'FORCES_NLP_solver/lib/FORCES_NLP_solver.so')) 
try:
...
csolver.argtypes = ( ctypes.POINTER(FORCES_NLP_solver_params_ctypes), ctypes.POINTER(FORCES_NLP_solver_outputs_ctypes), ctypes.POINTER(FORCES_NLP_solver_info), ctypes.POINTER(FILE))
...
a Python wrapper for a fast solver generated by FORCESPRO v5.0.1
 ...
 See also COPYING
    ...
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.FORCES_NLP_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), PyFile_AsFile(fp)  )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.FORCES_NLP_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), None )
            else:
                exitflag = _lib.FORCES_NLP_solver_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), fp )
            libc.fclose(fp)
