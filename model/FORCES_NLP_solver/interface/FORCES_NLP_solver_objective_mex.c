...
typedef FORCES_NLP_solver_float solver_float;
...
void copyCArrayToM_FORCES_NLP_solver(FORCES_NLP_solver_float *src, double *dest, solver_int32_default dim) 
{
...
void copyCValueToM_FORCES_NLP_solver(FORCES_NLP_solver_float* src, double* dest)
{
    *dest = (double)*src;
}

void copyMArrayToC_FORCES_NLP_solver(double *src, FORCES_NLP_solver_float *dest, solver_int32_default dim) 
{
    solver_int32_default i;
    for( i = 0; i < dim; i++ ) 
    {
...
void copyMValueToC_FORCES_NLP_solver(double * src, FORCES_NLP_solver_float * dest)
{
	*dest = (FORCES_NLP_solver_float) *src;
}
...
{
    const solver_int nvarArr[NSTAGES] = {8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8, 8}; 
    const solver_int neqArr[NSTAGES] = {6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6, 6};
    ...
    const solver_int dimlArr[NSTAGES] = {2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2};
    const solver_int dimuArr[NSTAGES] = {2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2};
...
void mexFunction( solver_int nlhs, mxArray *plhs[], solver_int nrhs, const mxArray *prhs[] )  
{
    ...
    arr = prhs[0];
    copyMArrayToC_FORCES_NLP_solver(mxGetPr(arr), z, nvar);
    if (nrhs > 1)
	{
        arr = prhs[1];
        if ( !mxIsEmpty(arr) )
        {
            copyMArrayToC_FORCES_NLP_solver(mxGetPr(arr), p, dimp);
        }
	}   
	...
	copyCArrayToM_FORCES_NLP_solver(obj, mxGetPr(obj_mex), 1);
	copyCArrayToM_FORCES_NLP_solver(jacobj, mxGetPr(gradobj_mex), nvar);
	plhs[0] = obj_mex;
	plhs[1] = gradobj_mex;
