...
# -*- coding: utf-8 -*-
"""
Created on Mon Nov 15 00:36:47 2021
...
# Definition of the informed RRT* search function
def Informed_RRT_star_path_search(x_start, x_goal, map_boundary): 
    '''
    Given the start position, goal position and map boundary, 
    Informed RRT* method is used to generate a waypoints list, 
    ...
    for iter in range(4000):
        ...
        c = norm(x_start-x_goal)/2
        ecc = 0.9
        a = c/ecc
#        b = np.sqrt(a**2-c**2)
#        t = np.linspace(0,2*np.pi,100)
#        theta = np.arctan( (x_goal[1]-x_start[1])/(x_goal[0]-x_start[0] ) )
#        center = (x_start+x_goal)/2
#        x = center[0] + a*np.cos(t)*np.cos(theta) - b*np.sin(t)*np.sin(theta)
#        y = center[1] + b*np.sin(t)*np.cos(theta) + a*np.cos(t)*np.sin(theta)
#        ax.plot(x,y, 'k', linewidth = 1)
        # random sampling in an ellipsoid
        x_rand = np.zeros(dim)
        x_rand[0] = random.random() * map_boundary
        x_rand[1] = random.random() * map_boundary
        x_rand[2] = random.random() * map_boundary
        if norm(x_rand-x_start) + norm(x_rand-x_goal) > 2*a:
            continue
...
path_list = Informed_RRT_star_path_search(x_start, x_goal, map_boundary)
...
ax.set_title('Informed RRT* Algorithm')
plt.show()
